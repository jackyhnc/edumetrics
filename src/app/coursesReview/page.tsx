    ...
    if (selectedCourse) {
      ...
      const loadTopics = async () => {
        try {
          ...
          const university = "Drexel";
          const subjects = await EduMetricsAPI.getSubjects(university, selectedCourse.name);
          setTopics(subjects);
        } catch (error) {
  ...
  const filteredCourses = coursesToDisplay
    .filter(course => {
      ...
      const matchesSubject = 
        subjectFilter === 'all' || 
        course.subjectCode.toLowerCase() === subjectFilter.toLowerCase();
      ...
      <div className="max-w-6xl mx-auto p-8 relative z-10">
        <div className="text-center mb-8 animate-fadeIn">
          ...
          <p className="mt-2 text-white/70 mb-6">
            Browse and share course information
          </p>
        </div>
          ...
          <div className="flex flex-wrap justify-center gap-4">
            ...
            >
              ...
              {subjectCodes.map(code => (
                <option key={code} value={code} className="bg-gray-900">{code}</option>
              ))}
            </select>
                      ...
                      >
                        ...
                        >
                          ...
                          <div className="flex justify-between text-sm text-white/50">
                            ...
                            <span>{course.crns.length} section{course.crns.length !== 1 ? 's' : ''}</span>
                          </div>
                        </button>
                      </div>
                    );
                    ...
                    <div className="bg-white/10 rounded-lg p-6 w-full shadow-lg shadow-purple-500/20 transition-all">
                      ...
                      {topics.length > 0 && (
                        <div className="mt-6 flex justify-center">
                          <Link 
                            href={`/AISummary?${selectedCourse ? 
                              `courseName=${encodeURIComponent(selectedCourse.name)}&courseTitle=${encodeURIComponent(selectedCourse.title)}${
